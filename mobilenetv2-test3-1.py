...
def MobilenetV2(input_data, trainable):
    with tf.variable_scope('MobilenetV2'):
        ...
        conv = common.inverted_residual(name='expanded_conv_1', input_data=conv, input_c=16, output_c=24, downsample=True,
                                 trainable=trainable)
        conv = common.inverted_residual(name='expanded_conv_2', input_data=conv, input_c=24, output_c=24, downsample=False,trainable=trainable)
        ...
        conv = common.inverted_residual(name='expanded_conv_5', input_data=conv, input_c=32, output_c=32,
                                          trainable=trainable)
        #
        conv = common.inverted_residual(name='expanded_conv_6', input_data=conv, input_c=32, output_c=64,
                                 downsample=False, trainable=trainable)
        ...
        conv = common.inverted_residual(name='expanded_conv_12', input_data=conv, input_c=96, output_c=96,
                                          trainable=trainable)
        #
        conv = common.inverted_residual(name='expanded_conv_13', input_data=conv, input_c=96, output_c=160,
        ...
        conv = common.convolutional(name='Conv_1', input_data=conv, filters_shape=(1, 1, 320, 1280),
                                      trainable=trainable, downsample=False, activate=True, bn=True)
    return conv


def ckpt_to_frozen_pb(sess,frozen_pb_file):
    frozen_graph_def = tf.graph_util.convert_variables_to_constants(sess, tf.get_default_graph().as_graph_def(),
...
def create_model():
    ...
    # feed_dict = {layer_input:img}
    # sess.run(out, feed_dict=feed_dict,options=options, run_metadata=run_metadata)
    # fetched_timeline = timeline.Timeline(run_metadata.step_stats)
    # chrome_trace = fetched_timeline.generate_chrome_trace_format()
    # with open('timeline_02.json', 'w') as f:
    #     f.write(chrome_trace)

    saver.save(sess, 'ckpt/456')
    return sess
...
if __name__ == "__main__":
    ...
    #print(out)
    sess = create_model()
    ckpt_to_frozen_pb(sess, '456.pb')

    cal_flops()
    cal_parameters()
    pb_performance('456.pb', RUN_TIMES)

# out2 = MobilenetV2(layer_input, trainable)
# with tf.Session() as sess:
#     # add additional options to trace the session execution
#     options = tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE)
#     run_metadata = tf.RunMetadata()
#     sess.run(out2, options=options, run_metadata=run_metadata)
#
