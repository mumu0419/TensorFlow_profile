...
out_list = []
x = tf.placeholder(tf.float32,[1,14,14,256])
group_num = 4
filters = 1024
# depth = x.shape[3]//group_num
b = tf.split(x,group_num,3)
print(len(b))

g_filters = filters/group_num
for i in range(len(b)):
    out = tf.layers.conv2d(b[i], g_filters,3,strides=1, padding='same')
    out_list.append(out)

print(out_list)
out = tf.concat(out_list,3)

print(out)

# for i in range(len(out_list)):
#     out = tf.concat(out_list,3)
# output = tf.layers.conv2d_transpose(x, 5, 3, strides=2, padding='same',kernel_initializer=tf.ones_initializer())
# print(output.shape)
#
